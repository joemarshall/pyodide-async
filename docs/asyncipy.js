...
class _MakeAsyncCalls(NodeTransformer):
	...
	def _parse_call(self,name):
		...
		while len(allNames)>0:
			retVal=Attribute(val=retVal,attr=allNames[0])
			allNames=allNames[1:]
		return retVal
...
function asyncipy_load()
{
	pyodide.runPython(`
	import js			
	import importlib.util
	spec = importlib.util.spec_from_loader('asyncipy', loader=None, origin="pyodide_init.py")
	asyncipy = importlib.util.module_from_spec(spec)
	exec(js.asyncipy_src, async_pyodide.__dict__)
	`)
}
